...
from tx import *
class Transmitter42b(Module):
	def __init__(self):
		self.data_in=Signal(42) #los bits a enviar
		self.trans_en=Signal() #habilitador para el envio
		self.tx_serial=Signal() #senhal serial de salida
		self.tx_42bdone=Signal() #se activa cuando se han enviado los dos bytes
		#  #  #
		...
		self.trans_fsm.act("WAITING",
			Case(self.byte_cnt,{
					...
					8: 	NextValue(transmitter.tx_data,self.data_in[40:42])
				}),
			If(~transmitter.tx_done,
				NextState("SENDING_BYTE")
			)
