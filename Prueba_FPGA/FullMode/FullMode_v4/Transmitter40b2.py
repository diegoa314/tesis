...
class Transmitter40b(Module):
	def __init__(self):
		...
		self.re = re = Signal()
		self.fifoEmpty = fifoEmpty = Signal()
		#  #  #
		...
		#self.comb+=self.re.eq(transmitter.tx_done)
		self.trans_fsm.act("IDLE",
			#NextValue(transmitter.tx_data,self.data_in[0:8]),
			If((self.trans_en & ~transmitter.tx_done) & ~fifoEmpty,
				NextState("DELAY"),
				...
				NextValue(re,0),
				NextValue(transmitter.tx_data,0b00000000)
			),
			NextValue(self.tx_40bdone,0)
		)
		self.trans_fsm.act("DELAY",
			NextValue(re,0),
			NextValue(self.aux,self.aux+1),
			If(self.aux == 2,	
