...
def reverse_table_flip(inputs, flips, nbits):
    ...
    for i, (word, flip) in enumerate(zip(inputs, flips)):
        ...
        outputs[word] = i
        if flip:
            word_n = ~word & (2**nbits-1) #(2**nbits-1)-word
...
table_5b6b_unbalanced = [bool(disparity(c, 6)) for c in table_5b6b] #true cuando esta desbalanceado
table_5b6b_flip = list(table_5b6b_unbalanced)
table_5b6b_flip[7] = True #111000 o 000111
...
class SingleEncoder(Module):
    def __init__(self, lsb_first=False):
        self.d = Signal(8)
        self.k = Signal()
        self.disp_in = Signal() #Si RD=-1, disp_in=0
